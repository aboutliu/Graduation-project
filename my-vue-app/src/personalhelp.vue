<template>
                ...
                <div class="resource-block" v-for="(resource, index) in currentresources" :key="index">
                    ...
                    <div class="resource-info">
                        <div class="resource-image">
                            <img :src="'http://localhost:8080' + '/images/' + (resource.image || 'help.png')" alt="活动图片" />
                        </div>
                        <p><strong>名称：</strong>{{ resource.name }}</p>
                        <p><strong>日期：</strong>{{ resource.time }}</p>
                        <p><strong>地点：</strong>{{ resource.place }}</p>
                        <p><strong>联系方式：</strong>{{ resource.phone }}</p>
  ...
  <div class="new-add-resource-block">
    ...
    <div class="resource-info">
      ...
      <input v-model="newresource.place" placeholder="地点" />
      <input v-model="newresource.phone" placeholder="联系方式" />
...
const currentresources = computed(() => {
  ...
  ? filtered.filter(act =>
      ...
      (act.phone && act.phone.toLowerCase().includes(searchQuery.value.toLowerCase())) ||
...
const deleteresource = async (id) => {
  try {
    ...
    // 先删除活动
    await axios.delete(`http://localhost:8080/help/${id}`);

    // 如果该活动有图片，发请求删除图片
    if (resource.image) {
      await axios.delete(`http://localhost:8080/help/deleteImage/${resource.image}`);
    }
...
//   alert(newresource.value.name);
  ...
  try {
    await axios.post('http://localhost:8080/help/uploadImage', formData);
    // alert('try');
...
const addresource = async () => {
  const { name, time, place, phone, introduction } = newresource.value;
  if (!name || !time || !place || !phone || !introduction) {
    // alert('请填写所有必填字段');
  ...
  try {
    await axios.post('http://localhost:8080/help', resourceToAdd);
    ...
    Object.assign(newresource.value, {
      name: '', time: '', place: '', phone: '', introduction: '', image: ''
    });
    uploadedImageName = '';
  } catch (error) {
    Object.assign(newresource.value, {
