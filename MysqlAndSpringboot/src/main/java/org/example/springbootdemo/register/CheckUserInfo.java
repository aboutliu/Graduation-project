...
public class CheckUserInfo {
    public ApiResponse validateUserInfo(User user) {
        ...
        if (user.getPassword() == null || user.getPassword().length() < 6) {
            return new ApiResponse(false, "密码长度至少为6位");
        }
        if (user.getAge() < 0) {
            return new ApiResponse(false, "年龄不能为负数");
        }
        if (user.getPhone() == null || !user.getPhone().matches("^1[3-9]\\d{9}$")) {
            return new ApiResponse(false, "手机号格式不正确");
        }
        if (user.getSex() == null || (!user.getSex().equals("男") && !user.getSex().equals("女"))) {
            return new ApiResponse(false, "性别只能是 男 或 女");
        }
